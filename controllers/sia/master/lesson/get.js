...
exports.get = async (req, res) => {
    try {
        ...
        const result = {
            ...
            total_page: Math.ceil(lessons.count / page_size),
            rows: lessons.rows
        }

        return response.success("Get all lesson success", res, result, 200);
    } catch (err) {
        console.log(err);
        return response.error(err.message || "Failed get all lesson", res);
    }
};
