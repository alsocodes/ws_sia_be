...
exports.get = async (req, res) => {
    try {
        ...
        const result = {
            total_count: eduyears.count,
            total_page: Math.ceil(eduyears.count / page_size),
            rows: eduyears.rows
        }

        return response.success("Get tahun ajaran sukses", res, result, 200);
    } catch (err) {
        console.log(err);
        return response.error(err.message || "Gagal get tahun ajaran ", res);
    }
};
